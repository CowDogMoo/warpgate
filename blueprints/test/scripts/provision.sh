...
configure_ssh() {
    run_as_root mkdir -p /home/ubuntu/.ssh
    run_as_root curl -s http://169.254.169.254/latest/meta-data/public-keys/0/openssh-key > /home/ubuntu/.ssh/authorized_keys
    run_as_root chown ubuntu:ubuntu /home/ubuntu/.ssh/authorized_keys
    run_as_root chmod 600 /home/ubuntu/.ssh/authorized_keys
    run_as_root usermod -s /bin/bash ubuntu
}
...
run_provision_logic() {
    ...
    ansible-playbook \
        ...
        --inventory 127.0.0.1, \
        --limit 127.0.0.1 \
        "${PKR_BUILD_DIR}/playbooks/attack-box/attack-box.yml"
