...
func (b *Blueprint) BuildPackerImages() (map[string]string, error) {
	...
	// Create S3 bucket for storing provision artifacts
	...
	// Create IAM resources with dynamic names
	profileName, roleName, err := packer.CreateIAMResources(cs.BucketName)
	if err != nil {
		return nil, fmt.Errorf("failed to create IAM resources: %v", err)
	}

	defer func() {
		if err := packer.DestroyIAMResources(profileName, roleName); err != nil {
			fmt.Printf("error destroying IAM resources: %v\n", err)
		}
	}()
	...
	go func(blueprint *Blueprint, pTmpl *packer.PackerTemplates) {
		...
		hashes, err := b.buildPackerImage(cs.BucketName, profileName)
		if err != nil {
...
// **Parameters:**
//
// bucketName: The name of the S3 bucket to store provision artifacts.
// profileName: The name of the IAM instance profile.
//
// **Returns:**
//
// []string: A slice of arguments for the packer build command.
func (b *Blueprint) PreparePackerArgs(bucketName, profileName string) []string {
	...
	args = append(args, b.appendAMIArgs(pTmpl, bucketName, profileName)...)
...
func (b *Blueprint) appendAMIArgs(pTmpl *packer.PackerTemplates, bucketName, profileName string) []string {
	var args []string
	amiConfig := pTmpl.AMI
	if amiConfig.InstanceType != "" {
		awsRegion := amiConfig.Region
		if awsRegion == "" {
			awsRegion = os.Getenv("AWS_DEFAULT_REGION")
			if awsRegion == "" {
				fmt.Println("Warning: AWS_DEFAULT_REGION environment variable is not set.")
			} else {
				fmt.Printf("Using AWS region from environment: %s\n", awsRegion)
			}
		}

		args = append(args, "-var", fmt.Sprintf("ami_region=%s", awsRegion))
		args = append(args, "-var", fmt.Sprintf("ansible_aws_ssm_bucket_name=%s", bucketName))
		args = append(args, "-var", fmt.Sprintf("iam_instance_profile=%s", profileName))
...
// map[string]string: A map of image hashes generated by the build process.
// error: An error if any issue occurs during the build process.
func (b *Blueprint) buildPackerImage(bucketName, profileName string) (map[string]string, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		hashes, err := b.BuildImageAttempt(attempt, bucketName, profileName)
		if err != nil {
			lastError = err
...
// error: An error if any issue occurs during the build process.
func (b *Blueprint) BuildImageAttempt(attempt int, bucketName, profileName string) ([]packer.ImageHash, error) {
	...
	args := b.PreparePackerArgs(bucketName, profileName)
