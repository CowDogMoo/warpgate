...
func (b *Blueprint) BuildPackerImages() (map[string]string, error) {
	...
	var cs *cloudstorage.CloudStorage

	// Only create S3 bucket if AMI builds are not disabled
	if !b.PackerTemplates.NoAMI {
		...
		bucketName = cs.BucketName

		// Setup cleanup for the bucket if we created one
		defer func() {
		...
		fmt.Println("Skipping S3 bucket creation as AMI builds are disabled")
	}
	...
	go func(blueprint *Blueprint, pTmpl *packer.PackerTemplates) {
		...
		hashes, err := b.buildPackerImage(bucketName)
		if err != nil {
...
func (b *Blueprint) PreparePackerArgs(bucketName string, noAMI, noContainers bool) []string {
...
// **Parameters:**
//
// bucketName: The name of the S3 bucket (may be empty if AMI builds are disabled).
//
// **Returns:**
//
// map[string]string: A map of image hashes generated by the build process.
...
func (b *Blueprint) BuildImageAttempt(attempt int, bucketName string) ([]packer.ImageHash, string, error) {
	...
	// Get the flags from the PackerTemplates
	noAMI := b.PackerTemplates.NoAMI
	noContainers := b.PackerTemplates.NoContainers

	args := b.PreparePackerArgs(bucketName, noAMI, noContainers)
