...
func (b *Blueprint) PreparePackerArgs(bucketName string) []string {
	...
	args := []string{
		...
		"-var", fmt.Sprintf("ansible_aws_ssm_bucket_name=%s", bucketName), // Use the created bucket name
	}
...
func (b *Blueprint) buildPackerImage(bucketName string) (map[string]string, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		hashes, err := b.BuildImageAttempt(attempt, bucketName)
		if err != nil {
			lastError = err
...
// error: An error if any issue occurs during the build process.
func (b *Blueprint) BuildImageAttempt(attempt int, bucketName string) ([]packer.ImageHash, error) {
